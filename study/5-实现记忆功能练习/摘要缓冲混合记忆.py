...
class ConversationSummaryBufferMemory:
    ...
    def __init__(self, summary: str = '', chat_histories: list = None, max_tokens: int = 300):
        ...
        self._client = OpenAI(base_url='https://api.moonshot.cn/v1',api_key=os.getenv("OPENAI_API_KEY"))
...
if not api_key:
    raise ValueError("请先在 .env 中配置 OPENAI_API_KEY")

client = OpenAI(base_url="https://api.moonshot.cn/v1", api_key=api_key)
# 创建记忆类
memory = ConversationSummaryBufferMemory("",chat_histories=[], max_tokens=300)
...
while True:
    ...
    try:
        ...
        # 拼接摘要+当前用户提问
        answer_prompt = (
            "你是一个强大的聊天机器人，请根据对应上下文和用户提问解决问题。\n\n"
            f"{memory_variables['chat_history']}\n\n"
            f"用户提问是:{query}"
        )
        messages = [
